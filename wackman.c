...
void compute_occurrence_array(int occurrence_array[ASCII_CHARACTER_SET_SIZE], char* string) {
    int i;
    for (i = 0; i < ASCII_CHARACTER_SET_SIZE; i++) {
...
void insert_sorted(WackyLinkedNode** head_ref, WackyLinkedNode* new_node) {
    if (*head_ref == NULL || 
        (*head_ref)->val->weight > new_node->val->weight || 
        ((*head_ref)->val->weight == new_node->val->weight && (*head_ref)->val->val > new_node->val->val)) {
        new_node->next = *head_ref;
        *head_ref = new_node;
    } else {
        WackyLinkedNode* current = *head_ref;
        while (current->next != NULL && 
               (current->next->val->weight < new_node->val->weight || 
                (current->next->val->weight == new_node->val->weight && current->next->val->val < new_node->val->val))) {
...
WackyLinkedNode* create_wacky_list(int occurrence_array[ASCII_CHARACTER_SET_SIZE]) {
    int total_occurrences = sum_array_elements(occurrence_array, ASCII_CHARACTER_SET_SIZE);
    
    WackyLinkedNode* head = NULL;

    for (int i = 0; i < ASCII_CHARACTER_SET_SIZE; i++) {
        if (occurrence_array[i] > 0) {
            double probability = (double)occurrence_array[i] / total_occurrences;
            WackyTreeNode* leaf_node = new_leaf_node(probability, (char)i);
            WackyLinkedNode* linked_node = new_linked_node(leaf_node);
            insert_sorted(&head, linked_node);
        }
    }
...
WackyTreeNode* merge_wacky_list(WackyLinkedNode* linked_list) {
    ...
    if (linked_list->next == NULL) {
        WackyTreeNode* root = linked_list->val;
        free(linked_list);
        return root;
    }
    ...
    WackyTreeNode* new_node = new_branch_node(first_node->val, second_node->val);

    WackyLinkedNode* new_link_node = new_linked_node(new_node);

    insert_sorted(&linked_list, new_link_node);

    free(first_node);
    free(second_node);

    return merge_wacky_list(linked_list);
}
...
void get_wacky_code(WackyTreeNode* tree, char character, bool boolean_array[], int* array_size) {
    if (tree == NULL || array_size == NULL) {
        *array_size = -1;
        return;
    }

    void traverse(WackyTreeNode* node, char target, int depth, int* current_size) {
        if (node == NULL) {
            return;
        }
        ...
        traverse(node->left, target, depth + 1, current_size);
        ...
        traverse(node->right, target, depth + 1, current_size);
    }

    *array_size = -1;
    traverse(tree, character, 0, array_size);
}
...
char get_character(WackyTreeNode* tree, bool boolean_array[], int array_size) {
    ...
    for (int i = 0; i < array_size; i++) {
        if (boolean_array[i] == false && tree->left != NULL) {
            tree = tree->left;
        }
        else if (boolean_array[i] == true && tree->right != NULL) {
            tree = tree->right;
        }
        else {
            return '\0';
